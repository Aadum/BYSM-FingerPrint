...
public class GlobalData {
	...
	public List<RecordItem> recordList1=null;
  ...
  //�����ظ��ж�  
    public boolean FindUserItemByUserItem(UserItem useritem){
   	for(int i=0;i<recordList.size();i++){
    		if(useritem.id.equals(recordList.get(i).id)){
    			long a=ExtApi.StrToDate(ExtApi.getStringDate()).getTime()-ExtApi.StrToDate(recordList.get(i).datetime).getTime();
    ...
    public void LoadRecordsList(){
    	recordList1=RecordFile.ReadFromFile(sDir+"/recordslist.dat");
    }
