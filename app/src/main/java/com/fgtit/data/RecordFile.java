...
public class RecordFile {
	...
	public static void AppendToFile(String fileName, RecordItem rs) {
		try {
			...
			System.arraycopy(rs.lastname.getBytes(), 0, content, 16, rs.lastname.getBytes().length);
	...
	public static ArrayList<RecordItem> ReadFromFile(String fileName){
		...
		try {
			...
			for(long i=0;i<count;i++){
				...
				rc.ward=new String(content, 16, 16);
				rc.ward=rc.ward.replaceAll("\\s","");
				rc.lastname=new String(content, 16, 16);
				rc.lastname=rc.lastname.replaceAll("\\s","");
				rc.lga=new String(content, 16, 16);
				rc.lga=rc.lga.replaceAll("\\s","");
