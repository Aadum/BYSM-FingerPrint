...
public class EnrollActivity extends AppCompatActivity {
    ...
    private EditText editText1, editText2, editText6, editText7, editText8, editText9, editText10, editTextLastName, editTextGravity, editTextParity;
...
//Spinner to generate LGA the actual list is in the string resource
        ...
        spinLga.setOnItemSelectedListener(new OnItemSelectedListener() {
            @Override
...
//Spinner to generate Ward the actual list is in the string resource
        ...
        spinWard.setOnItemSelectedListener(new OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View arg1, int pos, long arg3) {
                //person.ident=pos;
            }
        ...
        switch (id) {
            ...
            case R.id.action_save: {
                if (CheckInputData(1)) {
                    ...
                    person.lastname = (editTextLastName.getText().toString());
                    person.lga = (spinLga.getSelectedItem().toString());
                    person.ward = (spinWard.getSelectedItem().toString());
                    if (isenrol1) {
...
// Todo: uncomment                   SerialPortManager.getInstance().closeSerialPort();
            ...
            return true;
//            This seems to be for encoding card.. will check later
    ...
    private boolean CheckInputData(int type) {
        ...
        if (len <= 0) {
            Toast.makeText(EnrollActivity.this, "ID cannot be null", Toast.LENGTH_SHORT).show();
            return false;
        }
