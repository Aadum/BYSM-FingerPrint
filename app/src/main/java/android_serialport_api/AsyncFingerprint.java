...
public class AsyncFingerprint extends Handler {
    ...
    private synchronized int PSGetImage() {
        ...
        sendCommand(command);
        int length = SerialPortManager.getInstance().read(buffer, 12, 600);
    ...
    private synchronized byte[] PSUpChar() {
        ...
        sendCommand(command);
        int length = SerialPortManager.getInstance().read(buffer, UP_CHAR_RESPONSE_SIZE, 600);
